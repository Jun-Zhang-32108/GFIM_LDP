...
class LR3():
	...
	def run(self):
		c_privacy  = (math.exp(self.Privacy)+1.0)/(math.exp(self.Privacy)-1.0)
		m = 1/(self.List_x[0]**2)
		if int(self.Location) == 9999:
			# print('0!')
			dummy_list = [-1,1]
			z =random.choice(dummy_list)*c_privacy*math.sqrt(m)
			return z
		else:		
			x_location = float(self.List_x[self.Location])
			seed = random.random()
			p1 = math.exp(self.Privacy)/(math.exp(self.Privacy)+1.0)
			if seed<p1:
				z = c_privacy*m*x_location
...
def S_Hist_part1_topk(Location_num,user_num,Locations_ID,Privacy_Speic,true_p,true_locations_number_sorted,csv_writer1,CandidateSet_Size,k):
	...
	for i in xrange(user_num):
		...
		x_pri = mart[j]
		LR_Test = LR3( x_pri, Locations_ID[i], Privacy_Speic)
		result_z = LR_Test.run()
		z_vector[j] += result_z
	for i in xrange(Location_num):
		e_vector = np.zeros(Location_num)
		e_vector[i] = 1
		f_vector[i] = int(np.dot(np.dot(mart,e_vector), z_vector))
	...
	print('The sum of estimated counts is %d. '%(int(estimated_sum*2*user_itemset_fixsized)))
	re = []
	IREL = Location_num
	candidate_set = {}
	...
	for i in xrange(CandidateSet_Size):
		...
		if true_p[i] == 0:
			...
			csv_writer1.writerow([true_locations_number_sorted[i][0],true_locations_number_sorted[i][1],f_vector_sorted[i][0],f_vector_sorted[i][1]*2*user_itemset_fixsized,true_p[i],dict_estimated_frequency.get(true_locations_number_sorted[i][0]),estimated_p,'none',REL,DCG,IDCG,'part1',currentTime,candidate_set.keys(),'None']) 
			...
			if i == 0:
				...
				DCG += REL/math.log(i+1,2)
				IDCG += IREL/math.log(i+1,2)
			csv_writer1.writerow([true_locations_number_sorted[i][0],true_locations_number_sorted[i][1],f_vector_sorted[i][0],f_vector_sorted[i][1]*2*user_itemset_fixsized,true_p[i],dict_estimated_frequency[true_locations_number_sorted[i][0]],estimated_p,re[-1],REL,DCG,IDCG,'part1',currentTime,candidate_set.keys(),'None'])
...
def S_Hist_part2_topk(Location_num,Candidate,user_num,user_num_total,Locations_ID,Privacy_Speic,true_p,true_locations_number_sorted,csv_writer1,dict_estimated_frequency_part1,k):
	...
	for i in xrange(user_num):
		...
		# print('J1:%d'%j)
		x_pri = mart[j]
		LR_Test = LR2( x_pri, Locations_ID[i], Privacy_Speic,Candidate_2LR)
		result_z, isin_temp = LR_Test.run()
		# print('Isin_temp:%d'%isin_temp)
		# print('Result_z:%f'%result_z)
		z_vector[j] += result_z
		isin_counter += isin_temp
	Fall_IN_Candidate_Perce = isin_counter/float(user_num) 
	...
	for i in xrange(Location_num):
		if i in f_vector:
			dict_estimated_frequency_part2[i] = f_vector[i]/float(user_num)*user_itemset_fixsized_new
			...
			dict_estimated_frequency_final[i] = dict_estimated_frequency_part2[i]
		else:
	...
	estimated_sum = sum(f_vector)
	print('The sum of estimated counts of heavy hitters is %d. '%(int(estimated_sum*2*user_itemset_fixsized)))
	re = []
	IREL = Location_num
	estimated_topk_set =set()
	true_topk_set =set()
...
if __name__ == '__main__':
	...
	fileout = 'F:\study_BUPT\Thesis\Code_Lite\Data\EstimatedData'+'//'+'new_20180522'+'//'+filename+'_estimated_lite_2parts.csv'
	...
	fileout3 = 'F:\study_BUPT\Thesis\Code_Lite\Data\EstimatedData'+'//'+'new_20180522'+'//'+'algorithm_log_complete.csv'
	...
	Locations_ID_Uni = {}
	for i in xrange(Location_num_Total):
	...
	user_num_half = int(User_num_Total/2.0) 

	user_num_left = User_num_Total - user_num_half
	k = 10		
	...
	for Pri_Par in Pri_Par_list:
		...
		for counter in xrange(20):
			...
			for row1 in cache[:user_num_half]:
				# print('Cache:')
				# print(row1)
			...
			Candidate, RE1, dict_estimated_frequency_part1_0,NDCG1,Precision1 = S_Hist_part1_topk(Location_num_Total,user_num_half,Locations_ID,Pri_Par,true_frequency,true_locations_number_sorted0,csv_writer1,CandidateSet_Size,k)
			...
			for row2 in cache[user_num_half:User_num_Total]:
				new_list = list(set(row2).intersection(CandidateSet))
				if len(new_list) < user_itemset_fixsized_2:
					new_list.extend([9999]*(user_itemset_fixsized_2-len(new_list)))
			...
			RE2,NDCG2,Precision2 = S_Hist_part2_topk(Location_num_Total,Candidate,user_num_left,User_num_Total,Locations_ID,Pri_Par,true_frequency,true_locations_number_sorted0,csv_writer1,dict_estimated_frequency_part1_0,k)
			RE_List1.append(RE1)
